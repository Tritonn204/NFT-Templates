 ...
 */
contract COTC2Customs is ERC721, ERC721Enumerable, ERC721URIStorage, NFTCombinationManagement, ClampedRandomizer, Ownable {
    ...
    function setCOTC2(address nftContract) public onlyOwner {
        _COTC2 = nftContract;
    }
    ...
    function mint(uint, uint256[] calldata chosenTraits, uint256[] calldata tokensToBurn) public {
        require(_COTC2 != address(0));
        ...
        require(tokensToBurn.length == _burnAmount);

        ERC721 burnerNFT = ERC721(_COTC2);

        for (uint256 i = 0; i < _burnAmount; i++) {
            require(burnerNFT.isApprovedForAll(msg.sender, address(this)) == true || burnerNFT.getApproved(tokensToBurn[i]) == address(this), "Contract has not yet been approved");
            require(burnerNFT.ownerOf(tokensToBurn[i]) == msg.sender);

            burnerNFT.safeTransferFrom(msg.sender, address(1), tokensToBurn[i]);
        }
